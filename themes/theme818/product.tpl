...
more info
* needs please refer to http://www.prestashop.com for rmation.
...
var doesntExist = '{l s='Esta combinação não existe para este produto. Por favor, selecione uma outra combinação.' js=1}';

var doesntExistNoMore = '{l s='Este produto não está mais em estoque' js=1}';

var doesntExistNoMoreBut = '{l s='com esses atributos, mas está disponível com os outros.' js=1}';

var uploading_in_progress = '{l s='Upload em andamento, por favor, seja paciente.' js=1}';

var fieldRequired = '{l s='Por favor, preencha todos os campos obrigatórios antes de salvar sua personalização.' js=1}';
...
<div id="primary_block" class="clearfix">
	...
	<div id="admin-action">
		...
		<p>{l s='Este produto não é visível para seus clientes.'}

		<input type="hidden" id="admin-action-product-id" value="{$product->id}" />

		<input type="submit" value="{l s='Publicar'}" class="exclusive btn btn-default" onclick="submitPublishProduct('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 0, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>

		<input type="submit" value="{l s='Voltar'}" class="exclusive btn btn-default" onclick="submitPublishProduct('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 1, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
...
<img id="bigpic" alt="{$product->name|escape:'htmlall':'UTF-8'}" title="{$product->name|escape:'htmlall':'UTF-8'}" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html'}" /><span class="mask"></span>	
		...
		{else}
			...
			<span id="view_full_size">
				...
				<span class="span_link">{l s='Maximizar'}</span>
		...
		{if isset($images) && count($images) > 2}<a id="view_scroll_left"  title="{l s='Ver outros'}" href="javascript:{ldelim}{rdelim}">{l s='Anterior'}</a>{/if}
				...
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}">
                                     ...
                                     {if $jqZoomEnabled}
                                       ...
                                       <a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html'}" class="cloud-zoom-gallery" title="{$image.legend|htmlspecialchars}" rel="useZoom: 'zoom1', smallImage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html'}'">

                                      		<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default')|escape:'html'}" alt="{$image.legend|htmlspecialchars}"  />
		...
		{if isset($images) && count($images) > 2}<a id="view_scroll_right" title="{l s='Ver outras'}" href="javascript:{ldelim}{rdelim}">{l s='Próximo'}</a>{/if}
        ...
        <p class="resetimg">
        	...
        	<a id="resetImages" href="{$link->getProductLink($product)|escape:'html'}" onclick="$('span#wrapResetImages').hide('slow');return (false);">{l s='Mostrar todas as imagens'}</a></span></p>
		...
		<ul id="usefull_link_block" class="clearfix" >
			...
			<li class="print"><a href="javascript:print();"><i class="icon-print"></i>{l s='Imprimir'}</a></li>
	...
	<div id="pb-left-column" class="col-xs-12 col-sm-7">
		...
		{*	<p class="buttons_bottom_block"><a href="javascript:{ldelim}{rdelim}" class="button btn btn-default">{l s='Mais detalhes'}</a></p>*}
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0} style="display:none;"{/if} action="{$link->getPageLink('cart')|escape:'html'}" method="post">
			...
			<p id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				...
				{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='Este produto não está mais em estoque'}{/if}{else}{$product->available_now}{/if}
			...
			<p id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}> 
				...
				<span {if $product->quantity > 1} style="display: none;"{/if} id="quantityAvailableTxt">{l s=' Item em estoque'}</span>

				<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s=' Produtos em estoque'}</span>
			...
			<p class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties OR $product->quantity <= 0) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Atenção: Último produto em estoque!'}</p>
                ...
                <p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}> 
                    ...
                    <span class="editable">{$product->reference|escape:'htmlall':'UTF-8'}</span>
                           ...
                           {if $product->online_only}
                            ...
                            <span class="online_only">{l s='Online'}</span>
			...
			<p id="minimal_quantity_wanted_p"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				...
				{l s='Este produto não é vendido individualmente. Você deve selecionar pelo menos'} <b id="minimal_quantity_label">{$product->minimal_quantity}</b> {l s='quantidade para este produto.'}
			...
			{/if}
             ...
             <div class="row-2" {if !$product->specificPrice} style="display:none;"{/if}>
					...
					<img src="{$img_dir}onsale_{$lang_iso}.gif" alt="{l s='À venda'}" class="on_sale_img"/>

					<span class="on_sale">{l s='À venda!'}</span>

                    {elseif $product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutReduction > $productPrice}

                        <span class="discount">{l s='Desconto!'}</span>
				...
				{/if}
                    ...
                    {else}
                        ...
                        <p id="add_to_cart" class="buttons_bottom_block">
                            ...
                            <a class="exclusive button btn btn-default btn_add_cart" href="javascript:document.getElementById('add2cartbtn').click();"> <span>{l s='Adicionar ao carrinho'}  </span></a>
                    ...
                    <div class="other-prices">
                         ...
                         {if $priceDisplay == 2}
                            ...
                            <span id="pretaxe_price"><span id="pretaxe_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='sem taxa'}</span>

                        {/if}

                        {if $packItems|@count && $productPrice < $product->getNoPackPrice()}

                            <p class="pack_price">{l s='Em vez de'} <span class="price price-old">{convertPrice price=$product->getNoPackPrice()}</span></p>

                        {/if}

                        {if $product->ecotax != 0}

                            <p class="price-ecotax">{l s='Incluir'} <span id="ecotax_price_display">{if $priceDisplay == 2}{$ecotax_tax_exc|convertAndFormatPrice}{else}{$ecotax_tax_inc|convertAndFormatPrice}{/if}</span> {l s='para imposto verde'}

                                {if $product->specificPrice AND $product->specificPrice.reduction}

                                <br />{l s='(não impactado pelo desconto)'}
                        ...
                        {if !empty($product->unity) && $product->unit_price_ratio > 0.000000}
                            ...
                            <p class="unit-price"><span id="unit_price_display">{convertPrice price=$unit_price}</span> {l s='por'} {$product->unity|escape:'htmlall':'UTF-8'}</p>
...
<section id="quantityDiscount" class="page_product_box toggle_frame">
	...
	<h3 class="toggle">{l s='Escalar preço por'}<i class="icon-toggle icon-minus-sign-alt"></i></h3>
            ...
            <table class="table table-bordered table-hover std shop_table footable ">
                ...
                <thead>
                    ...
                    <tr>
                        ...
                        <th data-class="expand">{l s='Produto'}</th>

                        <th data-hide="phone">{l s='a partir de (qty)'}</th>

                        <th data-hide="phone">{l s='Desconto'}</th>
...
<section class="page_product_box toggle_frame more_info_inner">
	...
	<h3 class="toggle">{l s='Mais informações'}<i class="icon-toggle icon-minus-sign-alt"></i></h3>
...
<section class="page_product_box toggle_frame datasheet">
	...
	<h3 class="toggle">{l s='Ficha de dados'}<i class="icon-toggle icon-minus-sign-alt"></i></h3>
...
<section class="page_product_box toggle_frame more_info_inner4">
	...
	<h3 class="toggle">{l s='Acessórios'}<i class="icon-toggle icon-minus-sign-alt"></i></h3>
						...
						{if ($accessory.allow_oosp || (isset($accessory.quantity_all_versions) && $accessory.quantity_all_versions > 0) || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="ajax_block_product row bordercolor {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description">
								...
								<div class="accessories_price col-xs-12 col-sm-3">
									...
									<a class="exclusive button btn btn-default ajax_add_to_cart_button btn_add_cart" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")|escape:'html'}" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Adicionar ao carrinho'}"><span>{l s='Adicionar ao carrinho'}</span></a>

                                    {else}

                                     <span class="exclusive disabled">{l s='Adicionar ao carrinho'}</span>

                                     <span class="availability">

                                        {if (isset($accessory.quantity_all_versions) && $accessory.quantity_all_versions > 0)}{l s='Produto disponível com diferentes opções'}{else}{l s='Fora de estoque'}{/if}
...
<section class="page_product_box toggle_frame">
	...
	<h3>{l s='Personalização do Produto'}<i class="icon-toggle icon-minus-sign-alt"></i></h3>
		...
		<div class="customization_block toggle_content">
			...
			<form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix">
				...
				<p class="infoCustomizable">
					...
					{l s='Depois de salvar o produto personalizado, não se esqueça de adicioná-lo ao seu carrinho.'}

					{if $product->uploadable_files}<br />{l s='Formatos de arquivo permitidos: GIF, JPG, PNG'}{/if}
				...
				<div class="customizableProductsFile titled_box clearfix">
							...
							{if $field.type == 0}
								...
								<li class="customizationUploadLine{if $field.required} required{/if}">{assign var='key' value='pictures_'|cat:$product->id|cat:'_'|cat:$field.id_customization_field}
									...
									<div class="customizationUploadBrowse">
										...
										<a  class="btn btn-danger" href="{$link->getProductDeletePictureLink($product, $field.id_customization_field)|escape:'html'}" title="{l s='Apagar'}" ><i class="icon-trash icon-large"></i>{l s='Apagar'}
					...
					<div class="form-group">
						...
						<label class="customizationUploadBrowseDescription">{if !empty($field.name)}{$field.name}{else}{l s='Por favor, selecione uma imagem do seu computador'}{/if}{if $field.required}<sup>*</sup>{/if}</label>
				...
				<p id="customizedDatas">
					...
					<input type="button" class="button btn btn-default" value="{l s='Salvar'}" onclick="javascript:saveCustomization()" />

					<span id="ajax-loader" style="display:none"><img src="{$img_ps_dir}loader.gif" alt="loader" /></span>

				</p><p class="required"><sup>*</sup> {l s='campos obrigatórios'}</p>
...
{/if}
    ...
    {if isset($packItems) && $packItems|@count > 0}
        ...
        <section id="blockpack" class="page_product_box toggle_frame">
            ...
            <h3 class="toggle">{l s='Pacote de conteúdo'}<i class="icon-toggle icon-minus-sign-alt"></i></h3>
