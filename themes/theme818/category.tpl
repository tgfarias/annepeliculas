...
{if isset($category)}
	...
	{if $category->id AND $category->active}
		...
		{/if}
			...
			{if strlen($category->description) > 480}
				...
				<p class="cat_desc clearfix" id="category_description_short">{$category->description|truncate:480}&nbsp;<span onclick="$('#category_description_short').hide(); $('#category_description_full').show();" class="lnk_more_cat"><i class="icon-plus-sign"></i> {l s='Mais'}</span></p>

			<p class="cat_desc clearfix" id="category_description_full" style="display:none">{$category->description}<span onclick="$('#category_description_short').show(); $('#category_description_full').hide();" class="lnk_more_cat close_cat"><i class="icon-minus-sign"></i> {l s='Esconder'}</span></p>
	...
	{elseif $category->id}
		...
		<p class="alert alert-info">{l s='Esta categoria está indisponível no momento.'}</p>
