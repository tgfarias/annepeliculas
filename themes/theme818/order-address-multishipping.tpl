...
// <![CDATA[
	...
	var txtSelectAnAddressFirst = "{l s='Por favor, comece por selecionar um endereço' js=1}";
...

	{
		...
		return {
			...
			'invoice': "{l s='Seu endereço de cobrança'}",

			'delivery': "{l s='Seu endereço de entrega'}"
...

	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")|addslashes}'+id_address+'&amp;back=order?step=1{if $back}&mod={$back}{/if}" title="{l s='Atualizar' js=1}">{l s='Atualizar' js=1}</a>';
...
{if !$opc}<h1>{l s='Endereços'}</h1>{else}<h2><span>1</span> {l s='Endereços'}</h2>{/if}
...
{/if}
	...
	<div class="addresses clearfix">
		...
		{else}
			...
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large btn btn-default">{l s='Add um novo endereço'}</a>
		...
		<p class="address_add submit">
			...
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back|urlencode}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large btn btn-default">&raquo; {l s='Add um novo endereço'}</a>
...
{if !$opc}
	...
	<p class="cart_navigation clearfix submit">
		...
		{if $back}
			...
			<a href="{$link->getPageLink('order', true, NULL, "step=0&amp;back={$back}")|escape:'html'}" title="{l s='Retornar'}" class="button btn btn-default">&laquo; {l s='Retornar'}</a>

		{else}

			<a href="{$link->getPageLink('order', true, NULL, "step=0")|escape:'html'}" title="{l s='Retornar'}" class="button btn btn-default">&laquo; {l s='Retornar'}</a>
