...
{capture name=path}{l s='Fornecedores:'}{/capture}

{include file="$tpl_dir./breadcrumb.tpl"}



<h1><span>{l s='Fornecedores:'}<strong>{strip}{if $nbSuppliers == 0}{l s='Não há fornecedores.'}{else}{if $nbSuppliers == 1}{l s='Existe %d fornecedor.' sprintf=$nbSuppliers}{else}{l s='Existem %d fornecedores.' sprintf=$nbSuppliers}{/if}{/if}{/strip}</strong></span></h1>
		...
		<li class="shop_box clearfix {if $supplier@first}first_item{elseif $supplier@last}last_item{else}item{/if}">
        	...
        	<div class="col-xs-12 col-sm-8 col-lg-9 border_sep">
				...
				{/if}
					...
					<span>{if $supplier.nb_products == 1}{l s='%d produto' sprintf=$supplier.nb_products|intval}{else}{l s='%d produtos' sprintf=$supplier.nb_products|intval}{/if}</span>
			...
			{if $supplier.nb_products > 0}
				...
				<a class="button btn btn-default" href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}">{l s='Ver produtos'}</a>
