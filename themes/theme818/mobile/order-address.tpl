...
// <![CDATA[
	...
	{
		return {
			'invoice': "<span style=\"font-weight:bold;font-size:1.2em;padding-bottom:5px;display:block\">{l s='Seu endereço de cobrança' js=1}</span>",
			'delivery': "<span style=\"font-weight:bold;font-size:1.2em;padding-bottom:5px;display:block\">{l s='Seu endereço de entrega' js=1}</span>"
		};
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")}'+id_address+'&amp;back={$back_order_page}?step=1{if $back}&mod={$back}{/if}" title="{l s='Atualizar' js=1}" style="font-size:.9em;font-weight:normal;padding:8px 0;display:block;text-align:right">&raquo; {l s='Atualizar' js=1}</a>';
	...
	<div data-role="content" id="content" class="cart">
		...
		{if !$opc}
			...
			{if !$multi_shipping && {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
				<div class="button_multishipping_mode" id="multishipping_mode_box">
					<div class="title">{l s='Múltiplos Frete'}</div>
			...
			{if {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
				<div class="address-form-multishipping">
					<div class="button_multishipping_mode" id="multishipping_mode_box">
						<div class="title">{l s='Múltiplos Frete'}</div>
						<div class="description">
							<input type="checkbox" id="multishipping_mode_checkbox" onchange="multishippingMode(this); return false;"/><label for="multishipping_mode_checkbox">{l s='Eu gostaria de especificar um endereço de entrega para cada produto encomendado.'}</label>
						</div>
		...
		{/if}
			<div class="addresses clearfix">
				<p class="address_delivery select">
					<label for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Escolha um endereço de cobrança:'}{else}{l s='Escolha um endereço de entrega:'}{/if}</label>
				...
				{if $addresses|@count > 1}
					<label for="id_address_invoice" class="strong">{l s='Escolha um endereço de cobrança:'}</label>
					...
					{else}
						<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button_large">{l s='Adicionar um novo endereço'}</a>
					{/if}
				</p>
			</div>
			<div class="addresses">
				...
				<p class="address_add submit">
					<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button_large" data-role="button" data-icon="plus">{l s='Adicionar um novo endereço'}</a>
				</p>
				{if !$opc}
				<div id="ordermsg" class="clearfix">
					<p class="txt">{l s='Se você gostaria de adicionar um comentário sobre seu pedido, por favor, escreva-o no campo abaixo.'}</p>
		...
		{if !$opc}
			<fieldset class="cart_navigation submit ui-grid-a">
				...
				<div class="ui-block-a"><a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Retornar'}" data-role="button" data-icon="back" data-ajax="false">&laquo; {l s='Retornar'}</a></div>
				<div class="ui-block-b"><input type="submit" name="processAddress" value="{l s='Próximo'}" class="exclusive" data-icon="check" data-iconpos="right" data-theme="b" /></div>
			</fieldset>
		</form>
		{else}
		</div>
