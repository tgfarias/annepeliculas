...
<p class="warning">{l s='Você não tem autorização para realizar um novo pedido.'}</p>
{else}
	...
	<div data-role="content" id="content" class="cart">
		...
		{else}
			...
			{if isset($isVirtualCart) && $isVirtualCart}
				<p class="warning">{l s='Não é necessário uso de transportadora.'}</p>
			{else}
				...
				{foreach $delivery_option_list as $id_address => $option_list}
					<label id="delivery_option">
						{if isset($address_collection[$id_address])}
							{l s='Escolha uma opção de transporte para este endereço:'} {$address_collection[$id_address]->alias}
						{else}
							{l s='Escolha uma opção de transporte'}
						{/if}
					</label>
					...
					<p class="warning" id="noCarrierWarning">
						{foreach $cart->getDeliveryAddressesWithoutCarriers(true) as $address}
							{if empty($address->alias)}
								{l s='Nenhuma transportadora disponível.'}
							{else}
								{l s='Nenhuma transportadora disponível para este endereço "%s".' sprintf=$address->alias}
							{/if}
							{if !$address@last}
							<br />
							{/if}
			...
			<div style="display: none;" id="extra_carrier"></div>
				...
				{if $giftAllowed}
				<h3 class="gift_title">{l s='Presente'}</h3>
				<p class="checkbox">
					<input type="checkbox" name="gift" id="gift" value="1" {if $cart->gift == 1}checked="checked"{/if} />
					<label for="gift">{l s='Gostaria que meu pedido fosse embalado para presente.'}</label>
					<br />
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					{if $gift_wrapping_price > 0}
						...
						{if $use_taxes}{if $priceDisplay == 1} {l s='(sem taxa)'}{else} {l s='(com taxa)'}{/if}{/if})
					{/if}
				</p>
		...
		{if $conditions AND $cms_id}
			<h3 class="condition_title">{l s='Termos de serviço'}</h3>
			<p class="checkbox">
				<input type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} />
				<label for="cgv">{l s='Concordo com os Termos de Serviço.'}</label> <a href="{$link_conditions}" class="iframe">{l s='(Leia os Termos de Serviço)'}</a>
			</p>
		...
		{if !$opc}
			<fieldset class="cart_navigation submit ui-grid-a">
				...
				<div class="ui-block-a"><a href="{$link->getPageLink($back_order_page, true, NULL, "step=1{if $back}&back={$back}{/if}")}" title="{l s='Retornar'}" data-role="button" data-icon="back" data-ajax="false">&laquo; {l s='Retornar'}</a></div>
				<div class="ui-block-b"><input type="submit" name="processCarrier" value="{l s='Próximo'}" class="exclusive" data-icon="check" data-iconpos="right" data-theme="b" data-ajax="false" /></div>
			</fieldset>
		</form>
		{else}
		</div>
