...
<div class="clearfix">
	...
	{if $start!=$stop && $pages_nb >= 1}
		...
		<li class="pagination_previous">
			<a {$no_follow_text} class="button_prev{if $p == 1} disabled{/if}" data-role="button" data-icon="arrow-l" data-iconpos="left" href="{if isset($p_previous)}{$link->goPage($requestPage, $p_previous)}{/if}" data-ajax="false">{l s='Anterior'}</a>
		</li>
		{if $start>3}
		...
		{/if}
			<li class="pagination_next">
				<a {$no_follow_text} class="button_next{if !isset($p_next)} disabled{/if}" data-role="button" data-icon="arrow-r" data-iconpos="right" href="{if isset($p_next)}{$link->goPage($requestPage, $p_next)}{/if}" data-ajax="false">{l s='Próximo'}</a>
			</li>
		</ul>
	{/if}
	</div>
