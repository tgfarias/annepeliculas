...
{capture assign='page_title'}{l s='Seu endereço'}{/capture}
...
<div data-role="content" id="content">
	<div>
		...
		<form action="{$link->getPageLink('address', true)}" method="post" id="add_address" data-ajax="false">
			<legend><h3>{if isset($id_address) && $id_address != 0}{l s='Seu endereço'}{else}{l s='Novo endereço'}{/if}</h3></legend>
			...
			<p>{l s='Você deve registrar pelo menos um número de telefone.'} <sup class="required">*</sup></p>
			<div class="text">
			...
			<div class="text">
				<label for="phone_mobile">{l s='Celular'}</label>
			...
			<p class="required text" id="adress_alias">
				...
				<input type="text" id="alias" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{else if isset($address->alias)}{$address->alias|escape:'htmlall':'UTF-8'}{else if isset($select_address)}{l s='Meu endereço'}{/if}" />
			</p>
			<div>
				...
				<button type="submit" data-theme="a" name="submitAddress" value="submit-value" id="submitAddress" >{l s='Salvar'}</button>
			</div>
		</form>
	</div>
