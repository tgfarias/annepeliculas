<form action="{$link->getPageLink('authentication', true)}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation">
		<h3>{l s='Informações pessoais'}</h3>
		...
		<p class="required">
			...
			<span>{l s='(mínimo 5 caracteres)'}</span>
		</p>
		<p class="select">
	...
	{if $b2b_enable}
	<fieldset class="account_creation">
		<h3>{l s='Informações sobre sua empresa'}</h3>
		<p class="text">
		...
		<p class="text">
			<label for="website">{l s='Site'}</label>
	...
	<fieldset class="account_creation">
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "vat_number"}
				<div id="vat_number" style="display:none;">
					<p class="text">
						<label for="vat_number">{l s='Número VAT'}</label>
			...
			{elseif $field_name eq "address1"}
				<p class="required text">
					...
					<span class="inline-infos">{l s='Rua, nome da companhia, etc.'}</span>
				</p>
			{elseif $field_name eq "address2"}
				<p class="text">
					...
					<span class="inline-infos">{l s='Apartamento, suite, unidade, prédio, andar, etc...'}</span>
				</p>
		...
		<p class="text">
			<label for="phone">{l s='Telefone'}</label>
			<input type="text" class="text" name="phone" id="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if}" />
		</p>
		<p class="text">
			<label for="phone_mobile">{l s='Celular'} <sup>*</sup></label>
			<input type="text" class="text" name="phone_mobile" id="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{/if}" />
		</p>
		<p class="required text" id="address_alias">
			<label for="alias">{l s='Atribuir um endereço para referência futura'} <sup>*</sup></label>
			<input type="text" class="text" name="alias" id="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{else}{l s='Meu endereço'}{/if}" />
		</p>
	</fieldset>
	...
	<p class="cart_navigation required submit">
		...
		<input type="submit" name="submitAccount" id="submitAccount" value="{l s='Registrar'}" class="exclusive" />
