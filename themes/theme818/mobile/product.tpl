	...
	<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post" data-ajax="false">
		...
		<div class="clearfix">
			...
			<div id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<label for="qty" class="">{l s='Quantidade:'}</label>
			...
			<div id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<label id="availability_label" class="">{l s='Disponibilidade:'}</label>
			...
			<p id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}>
				...
				<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='Ítens em estoque'}</span>
			</p>
			{/if}
			
			{* à checker avec JS *}
			{* ================================== *}
			<p class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties OR $product->quantity <= 0) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Aviso: Últimos artigos em estoque!'}</p>
			{* ================================== *}
		...
		<div id="add_to_cart" class="btn-row">
			...
			{/if}
			<button type="submit" data-theme="{$cart_btn_theme}" name="Submit" class="{$cart_btn_class}" value="submit-value" id="Submit" {$cart_btn_icon} >{l s='Adicionar ao carrinho'}</button>
		</div><!-- .btn-row -->
	</form><!-- #buy_block -->
	{/if}
