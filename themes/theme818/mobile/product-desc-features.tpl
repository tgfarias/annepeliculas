			...
			<ul>
				{foreach from=$accessories item=accessory name=accessories_list}
					...
					<li class="ajax_block_product {if $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description clearfix">
						...
						{/if}
						<div class="btn-row">
							<a class="" data-theme="a" data-role="button" data-mini="true" data-inline="true" data-icon="arrow-r" href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{l s='Ver'}" data-ajax="false">{l s='Ver'}</a>
							...
							{else}
								...
								{capture assign="btn_more"}<span class="availability">{if (isset($accessory.quantity_all_versions) && $accessory.quantity_all_versions > 0)}{l s='Produto disponível com diferentes opções'}{else}{if !$PS_CATALOG_MODE}{l s='Fora de estoque'}{/if}{/if}</span>{/capture}
							{/if}
							<a class="{$btn_class}" data-role="button" data-inline="true" data-theme="e" data-icon="plus" data-mini="true" class="exclusive button ajax_add_to_cart_button" href="{$btn_href}" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Adicionar ao carrinho'}" data-ajax="false">{l s='Adicionar ao carrinho'}</a>
							{$btn_more}
						</div><!-- .btn-row -->
					</li>
				{/foreach}
