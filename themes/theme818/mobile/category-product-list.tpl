			...
			<a href="{$product.link|escape:'htmlall':'UTF-8'}" data-ajax="false">
				...
				{if (!$PS_CATALOG_MODE AND ((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}
					...
					{if isset($product.on_sale) && $product.on_sale && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}
						{capture assign='info3_str'}{l s='À Venda!'}{/capture}
					{elseif isset($product.reduction) && $product.reduction && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}
						{capture assign='info3_str'}{l s='Desconto!'}{/capture}
						{assign var='info3_class' value='discount'}
					{/if}
					<p class="ui-li-price-info {$info3_class}"><span>{$info3_str}</span></p>
					<p class="availability">
					{if isset($product.available_for_order) && $product.available_for_order && !isset($restricted_country_mode)}
						{if ($product.allow_oosp || $product.quantity > 0)}{l s='Disponível'}{elseif (isset($product.quantity_all_versions) && $product.quantity_all_versions > 0)}{l s='Produto disponível com diferentes opções'}{else}{l s='Fora de estoque'}{/if}
					{else}
						&nbsp;
					{/if}
					</p>
					...
					{if isset($product.online_only) && $product.online_only}
						<p class="online_only">{l s='Online!'}</p>
					{/if}
				{/if}
				{if isset($product.new) && $product.new == 1}<p class="new">{l s='Novo'}</p>{/if}
			</a>
		</li>
	{/foreach}
	</ul><!-- #category-list -->
