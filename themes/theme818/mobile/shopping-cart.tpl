...
{else}
	...
	<div data-role="content" id="content" class="cart">
		...
		<ul data-role="listview" data-inset="true" data-split-theme="d" data-split-icon="delete">
				...
				<li id="element_product_{$product.id_product}">
					<a>
						...
						<table class="width-100">
							<thead>
								<tr>
									<td class="width-40">{l s='Preço unit.'}</td>
									<td>{l s='Qnt.'}</td>
					...
					<a rel="nofollow" class="cart_quantity_delete" id="{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery|intval}&amp;token={$token_cart}")}" data-ajax="false">{l s='Apagar'}</a>
				</li>
			{/foreach}
			...
			{foreach $gift_products as $product}
				<li id="element_product_{$product.id_product}">
					...
					<a rel="nofollow" class="cart_quantity_delete" id="{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery|intval}&amp;token={$token_cart}")}" data-ajax="false" style="display:none">{l s='Apagar'}</a>
				</li>
			{/foreach}
		</ul>
		{/if}
		{if sizeof($discounts)}
		<h2>{l s='Lista de Cupons'}</h2>
		<ul data-role="listview" data-inset="true" data-split-theme="d" data-split-icon="delete">
			...
			<li>
				...
				{if strlen($discount.code)}<a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}" class="price_discount_delete" title="{l s='Apagar'}" data-ajax="false">{l s='Apagar'}</a>{/if}
			</li>
			{/foreach}
		</ul>
		{/if}
		...
		<div class="ui-grid-a cart_total_bar same-height">
			<div class="ui-block-a">
				<div class="ui-bar ui-bar-c">
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" data-ajax="false">
						<input type="text" name="discount_name" id="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}"  placeholder="{l s='Código do Cupom:'}" />
						<div class='btn-row'>
							<input type="hidden" name="submitDiscount" />
							<button type="submit" data-theme="a" name="submitAddDiscount" value="submit-value">{l s='Enviar'}</button>
						</div><!-- .btn-row -->
					</form>
				</div>
			</div>
			<div class="ui-block-b total_price">
				<div class="ui-bar ui-bar-c">
					{if $use_taxes}
						{if $priceDisplay}
							<h3>{if $display_tax_label}{l s='Total de Produtos (sem taxa)'}{else}{l s='Total de Produtos'}{/if}</h3>
							<p><span class="price" id="total_product">{displayPrice price=$total_products}</span></p>
						{else}
							<h3>{if $display_tax_label}{l s='Total de Produtos (com taxa)'}{else}{l s='Total de Produtos'}{/if}</h3>
							<p><span class="price" id="total_product">{displayPrice price=$total_products_wt}</span></p>
						{/if}
					{else}
						<h3>{l s='Total de Produtos'}</h3>
						<p><span class="price" id="total_product">{displayPrice price=$total_products}</span></p>
					{/if}

					<div {if $total_discounts == 0}class="hide"{/if}>
						{if $use_taxes && $display_tax_label}
							<h3>{l s='Total de Cupons (sem taxa)'}</h3>
						{else}
							<h3>{l s='Total de Cupons'}</h3>
						{/if}
					...
					<div {if $total_wrapping == 0}class="hide"{/if}>
						<h3>
						{if $use_taxes}
							{if $display_tax_label}{l s='Total da embalagem p/ presente (com taxa):'}{else}{l s='Custo total da embalagem p/ presente:'}{/if}
						{else}
							{l s='Custo total da embalagem p/ presente:'}
						{/if}
						</h3>
					...
					{if $use_taxes}
						{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
							<h3>{l s='Frete'}</h3>
							<p><span class="price" id="total_shipping">{l s='Frete grátis!'}</span></p>
						{else}
							{if $priceDisplay}
								<div {if $total_shipping_tax_exc <= 0}class="hide"{/if}>
									<h3>{if $display_tax_label}{l s='Total do frete (sem taxa)'}{else}{l s='Total do frete'}{/if}</h3>
							...
							{else}
								<div {if $total_shipping <= 0}class="hide"{/if}>
									<h3>{if $display_tax_label}{l s='Total do frete (com taxa)'}{else}{l s='Total do frete'}{/if}</h3>
		...
		{if $opc && $isLogged && !$isGuest}
			<a href="{$link->getPageLink('index', true)}" data-role="button" data-theme="a" data-icon="back" data-ajax="false">{l s='Continue comprando'}</a>
		{else}
			<ul data-role="listview" data-inset="true" id="list_myaccount">
				<li data-theme="a" data-icon="back">
					<a href="{$link->getPageLink('index', true)}" data-ajax="false">{l s='Continue comprando'}</a>
				</li>
				<li data-theme="b" data-icon="check">
					<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" data-ajax="false">{l s='Confirmar pedido'}</a>
				</li>
