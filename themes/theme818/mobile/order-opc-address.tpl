...
<div data-role="content" id="address-section">
	<div class="ui-grid-a margin-bottom-10px">
		<div class="ui-block-a">
			...
			{/if}
			<label for="delivery-address-choice" class="select">{l s='Mudança de endereço:'}</label>
			<select
		...
		<div class="ui-block-b">
			<h3 class="bg">{l s='Endereço de cobrança'}</h3>
			{if isset($invoice)}
				<ul class="adress">
			...
			{else}
				<p class="warning">{l s='Você deve especificar o endereço de  entrega e cobrança'}</p>
			{/if}
			<label for="invoice-address-choice" class="select">{l s='Mudança de endereço:'}</label>
			<select
	...
	<p><a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Adicionar um novo endereço'}" data-role="button" data-theme="e" data-icon="plus" data-ajax="false">{l s='Adicionar um novo endereço'}</a><br /></p>
