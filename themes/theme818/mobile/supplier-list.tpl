...
{capture assign='page_title'}{l s='Fornecedores:'}{/capture}
...
{else}
	<p class="nbrmanufacturer">{strip}
		<span class="bold">
			{if $nbSuppliers == 0}
				{l s='Não existem fornecedores.'}
			{else}
				{if $nbSuppliers == 1}
					{l s='Existe %d fornecedor.' sprintf=$nbSuppliers}
				{else}
					{l s='Existem %d fornecedores.' sprintf=$nbSuppliers}
				{/if}
			{/if}
		</span>{/strip}
	</p>
	...
	{foreach $suppliers_list as $supplier}
		<li data-corners="false" data-shadow="false" data-iconshadow="true" data-inline="false" data-wrapperels="div" data-icon="arrow-r" data-iconpos="right" data-theme="c" class="clearfix {if $supplier@first}first_item{elseif $supplier@last}last_item{else}item{/if}">
			...
			{if $supplier.nb_products == 1}
				{l s='%d produto' sprintf=$supplier.nb_products|intval}
			{else}
				{l s='%d produtos' sprintf=$supplier.nb_products|intval}
			{/if}
			</p>
