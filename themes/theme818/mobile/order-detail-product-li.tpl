...
{*if isset($product.customizedDatas)}
	<li class="item">
		...
		<p><strong>{l s='Referência'}</strong> {if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}--{/if}</p>
		<p><strong>{l s='Quantidade'}</strong></p>
		...
		<p><strong>{l s='Valor unit.'}</strong> 
		{if $group_use_tax}
		...
		<p><strong>{l s='Valor total'}</strong> 
	...
	<li class="item" id="cb-{$product.id_order_detail|intval}" data-icon="back">
		...
		<h3>
			{if $product.download_hash && $invoice && $product.display_filename != ''}
				...
				<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&amp;id_order={$order->id}&secure_key={$order->secure_key}")}" title="{l s='Baixar este produto'}" data-ajax="false">
				{else}
					<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")}" title="{l s='Baixar este produto'}" data-ajax="false">
				{/if}
