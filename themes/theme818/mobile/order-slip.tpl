...
{capture assign='page_title'}{l s='Créditos'}{/capture}
...
<div data-role="content" id="content">
	...
	<div class="block-center" id="block-history">
			...
			{foreach from=$ordersSlip item=slip name=myLoop}
				<li>
					...
					<a class="color-myaccount" id="order-{$id_order}" href="{$link->getPageLink('order-detail', true, null, "id_order=$id_order")}" data-ajax="false">
						<h3>{l s='Créditos'} {l s='#'}{$slip.id_order_slip|string_format:"%06d"}</h3>
						<p>{l s='Pedido'} {l s='#'}{$slip.id_order|string_format:"%06d"}</p>
						<span class="ui-li-aside">{dateFormat date=$slip.date_add full=0}</span>
					</a>
					<a rel="external" data-iconshadow="false" href="{$link->getPageLink('pdf-order-slip', true, NULL, "id_order_slip={$slip.id_order_slip|intval}")}" title="{l s='Crédito'} {l s='#'}{$slip.id_order_slip|string_format:"%06d"}" data-ajax="false">
						{l s='PDF'}
					</a>
				</li>
			{/foreach}
		...
		{else}
			<p class="warning">{l s='Você não tem crédito para receber.'}</p>
		{/if}
	</div>
</div><!-- /content -->
