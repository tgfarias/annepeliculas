...
{capture name=path}
	...
	{if !isset($email_create)}{l s='Autenticação'}{else}

		<a href="{$link->getPageLink('authentication', true)|escape:'html'}" rel="nofollow" title="{l s='Autenticação'}">{l s='Autenticação'}</a>

		<span class="navigation-pipe">{$navigationPipe}</span>{l s='Crie sua conta'}
...
<h1><span>{if !isset($email_create)}{l s='Autenticação'}{else}{l s='Crie sua conta'}{/if}</span></h1>
...
{if !isset($email_create)}
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="create-account_form" class="std col-xs-12 col-sm-6">
		...
		<fieldset class="titled_box">
			...
			<h2><span>{l s='Crie sua conta'}</span></h2>

			<div class="form_content clearfix">

				<p class="title_block">{l s='Digite seu e-mail para criar uma conta.'}</p>
				...
				<p class="submit form-group">
					...
					<input type="submit" id="SubmitCreate" name="SubmitCreate" class="button_large btn btn-default" value="{l s='Criar uma conta'}" />
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="login_form" class="std col-xs-12 col-sm-6">
		...
		<fieldset class="titled_box">
			...
			<h2><span>{l s='Já está registrado?'}</span></h2>
			...
			<div class="form_content clearfix">
				...
				<div class="form-group">
					...
					<label for="passwd">{l s='Senha'}</label>
				...
				<div class="lost_password form-group"><a href="{$link->getPageLink('password')|escape:'html'}" title="{l s='Recupere sua senha'}" rel="nofollow">{l s='Esqueceu sua senha?'}</a></div>

				<p class="submit">

					{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}

					<input type="submit" id="SubmitLogin" name="SubmitLogin" class="button btn btn-default" value="{l s='Autenticar'}" />
	...
	<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html'}" method="post" id="new_account_form" class="std clearfix">
		...
		<fieldset>
			...
			<h1><span>{l s='Checkout instantâneo'}</span></h1>
            ...
            <div class="col-xs-12 col-sm-6">
				...
				<h1><span>{l s='Endereço de Entrega'}</span></h1>
				...
				{foreach from=$dlv_all_fields item=field_name}
                        ...
                        <div class="col-xs-12 col-sm-6">
							...
							<input type="text" class="form-control cep" name="postcode" id="postcode" value="{if isset($smarty.post.postcode)}{$smarty.post.postcode}{/if}" onblur="$('#postcode').val($('#postcode').val().toUpperCase());" />
						...
						<p class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
							...
							<label for="phone">{l s='Telefone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}</label>

							<input type="text" class="form-control phone" name="phone" id="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if}"/>
		...
		<p class="cart_navigation clearfix required submit">
			...
			<span><sup>*</sup>{l s='Campos obrigatórios'}</span>

            <input type="hidden" name="display_guest_checkout" value="1" />

			<input type="submit" class="exclusive btn btn-default" name="submitGuestAccount" id="submitGuestAccount" value="{l s='Continuar'}" />
...
<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation titled_box">
		...
		<h2><span>{l s='Suas informações pessoais'}</span></h2>
        ...
        <div class="row">
			...
			<label for="customer_firstname">{l s='Nome'} <sup>*</sup></label>
			...
			<label for="customer_lastname">{l s='Sobrenome'} <sup>*</sup></label>
			...
			<input onkeyup="$('#lastname').val(this.value);" type="text" class="form-control" id="customer_lastname" name="customer_lastname" value="{if isset($smarty.post.customer_lastname)}{$smarty.post.customer_lastname}{/if}" />
             ...
             <div class="col-xs-12 col-sm-6">
                ...
                <p class="required form-group clearfix">
                    ...
                    <label for="passwd">{l s='Senha'} <sup>*</sup><span class="form_info">{l s='(Mínimo 5 caracteres)'}</span></label>
	...
	<fieldset class="account_creation titled_box">
		...
		<h2><span>{l s='Informações sobre sua empresa'}</span></h2>

		<p class="form-group">

			<label for="">{l s='Companhia'}</label>
	...
	<fieldset class="account_creation">
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "address1"}
				...
				<p class="required form-group">
					...
					<span class="inline-infos">{l s='Endereço, P.O. Box, nome da empresa, etc. '}</span>
			...
			{elseif $field_name eq "address2"}
				...
				<p class="form-group">
					...
					<span class="inline-infos">{l s='Apartamento, suite, unidade, construção, piso, etc ...'}</span>
		...
		<p class="textarea">
			...
			<label for="other">{l s='Informações Adicionais'}</label>
		...
		<p class="inline-infos">{l s='Você deve registrar pelo menos um número de telefone.'}</p>

        		{/if}

		<p class="form-group">

			<label for="phone">{l s='Telefone Residencial'}</label>
		...
		<p class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if} form-group">
			...
			<label for="phone_mobile">{l s='Telefone Celular'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}</label>
		...
		<p class="required form-group" id="address_alias">
			...
			<label for="alias">{l s='Atribuir um endereço para referência futura'} <sup>*</sup></label>
	...
	<p class="cart_navigation required submit form-group">
		...
		<input type="submit" name="submitAccount" id="submitAccount" value="{l s='Registrar'}" class="exclusive btn btn-default" />

		<span><sup>*</sup>{l s='Campo obrigatório'}</span>
