...
<form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
	...
	<div class="titled_box">
        ...
        <span>
			...
			{l s='Ordem de Referência %s - placed on' sprintf=$order->getUniqReference()} {dateFormat date=$order->date_add full=0}
...
{if $carrier->id}<p class="tit-ord"><strong>{l s='Portador'}</strong> {if $carrier->name == "0"}{$shop_name|escape:'htmlall':'UTF-8'}{else}{$carrier->name|escape:'htmlall':'UTF-8'}{/if}</p>{/if}

<p class="tit-ord"><strong>{l s='Forma de pagamento'}</strong> <span class="color-myaccount">{$order->payment|escape:'htmlall':'UTF-8'}</span></p>

{if $invoice AND $invoiceAllowed}

<p>

	<a class="btn btn-success" target="_blank" href="{$link->getPageLink('pdf-invoice', true)}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}">{l s='Baixe sua fatura em PDF.'}</a>
...
<p  class="tit-ord"><img src="{$img_dir}icon/recyclable.png" alt="" class="icon" />&nbsp;{l s='Você deu permissão para receber seu pedido em embalagens recicladas.'}</p>

{/if}

{if $order->gift}

	<p><i class="icon-gift"></i>&nbsp;{l s='Pedido embalado para presente.'}</p>
...
<h2><span>{l s='Siga o status do seu pedido passo-a-passo'}</span></h2>
	...
	<table class="detail_step_by_step table table-hover table-bordered">
		...
		<thead>
			...
			<tr>
				...
				<th class="first_item">{l s='Data'}</th>
...
<div class="adresses_bloc row">
	...
	<div class="col-xs-12 col-sm-6">
        ...
        <ul class="address item {if $order->isVirtual()}full_width{/if}">
            ...
            <li class="address_title">{l s='Faturamento'}</li>
    ...
    <div class="col-xs-12 col-sm-6">
        ...
        <ul class="address alternate_item" {if $order->isVirtual()}style="display:none;"{/if}> 
            ...
            <li class="address_title">{l s='Entrega'}</li>
	...
	<table id="order-detail-content-table" class="std table table-bordered shop_table footable table-hover">
		...
		<thead>
			...
			<tr>
                ...
                <th data-hide="phone,tablet" class="{if $return_allowed}item{else}first_item{/if}">{l s='Referência'}</th>

            	<th  data-class="expand" class="item">{l s='Produto'}</th>

				<th class="item">{l s='Quantidade'}</th>

				{if $order->hasProductReturned()}

					<th data-hide="phone,tablet" class="item">{l s='Devolvido'}</th>

				{/if}

				<th data-hide="phone,tablet" class="item">{l s='Valor unitário'}</th>

				<th data-hide="phone,tablet" class="last_item">{l s='Total'}</th>
		...
		<tfoot>
			...
			{if $priceDisplay && $use_tax}
				...
				<tr class="item">
					...
					<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
						...
						<strong>{l s='Total de produtos (sem taxa)'} </strong><span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithoutTaxes() currency=$currency}</span>
			...
			<tr class="item">
				...
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					...
					<strong>{l s='Total de produtos'} {if $use_tax}{l s='(com taxa)'}{/if}: </strong> <span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
			...
			<tr class="item">
				...
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					...
					<strong>{l s='Total de vales:'} </strong> <span class="price-discount">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency convert=1}</span>
			...
			<tr class="item">
				...
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					...
					<strong>{l s='O custo total de embrulho:'} </strong> <span class="price-wrapping">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
			...
			<tr class="item">
				...
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					...
					<strong>{l s='Total de envio'} {if $use_tax}{l s='(com taxa)'}{/if}: </strong> <span class="price">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
						...
						<td class="bold">
							...
							<label for="cb_{$product.id_order_detail|intval}">
								...
								{if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0}
									...
									<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&amp;id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html'}" title="{l s='Baixar este produto'}">

									{else}

										<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")|escape:'html'}" title="{l s='Baixar este produto'}">

									{/if}

										<img src="{$img_dir}icon/download_product.gif" class="icon" alt="{l s='Baixar produto'}" />

									</a>

									{if isset($is_guest) && $is_guest}

										<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html'}" title="{l s='Baixar este produto'}"> {$product.product_name|escape:'htmlall':'UTF-8'} 	</a>

									{else}

									<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")|escape:'html'}" title="{l s='Baixar este produto'}"> {$product.product_name|escape:'htmlall':'UTF-8'} 	</a>
		...
		{foreach from=$discounts item=discount}
			...
			<tr class="item">
				...
				<td>{l s='Vale'} {$discount.name|escape:'htmlall':'UTF-8'}</td>
...
{if $return_allowed}
	...
	<div id="returnOrderMessage" class="titled_box">
		...
		<h2><span>{l s='Devolução de mercadoria'}</span></h2>

		<p class="textarea">

        	<label>{l s='Se você deseja retornar um ou mais produtos, por favor, marque as caixas correspondentes e fornecer uma explicação para o retorno. Em seguida, clique no botão abaixo.'}</label>
	...
	<table id="shipping-table" class="table table-bordered shop_table footable table-hover">
		...
		<thead>
			...
			<tr>
				...
				<th data-class="expand"  class="first_item">{l s='Data'}</th>

				<th data-hide="phone,tablet" class="item">{l s='Portador'}</th>

				<th data-hide="phone,tablet" class="item">{l s='Peso'}</th>

				<th data-hide="phone,tablet" class="item">{l s='Custo de envio'}</th>

				<th data-hide="phone,tablet" class="last_item">{l s='Número de rastreamento'}</th>
...
{if !$is_guest}
		...
		<table class="detail_step_by_step table table-hover table-bordered">
			...
			<thead>
				...
				<tr>
					...
					<th class="first_item">{l s='Para'}</th>
	...
	<p class="alert alert-suces">
		...
		{l s='Mensagem enviada com sucesso!'}
	...
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std titled_box" id="sendOrderMessage">
		...
		<h2><span>{l s='Add uma mensagem'}</span></h2>
		...
		<p class="submit">
			...
			<input type="submit" class="btn btn-default" name="submitMessage" value="{l s='Enviar'}"/>
...
<p><i class="icon-info-sign"></i>&nbsp;{l s='Você não pode retornar a mercadoria com uma conta de convidado'}</p>
