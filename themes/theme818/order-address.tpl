...
// <![CDATA[
...

	{
		...
		return {
			...
			'invoice': "{l s='Seu endereço de cobrança' js=1}",

			'delivery': "{l s='Seu endereço de entrega' js=1}"
...

	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")|addslashes}'+id_address+'&amp;back={$back_order_page}?step=1{if $back}&mod={$back}{/if}" title="{l s='Atualizar' js=1}">&raquo; {l s='Atualizar' js=1}</a>';
...
{capture name=path}{l s='Endereços'}{/capture}
...
{if !$opc}<h1><span>{l s='Endereços'}</span></h1>{else}<h1><span>1 {l s='Endereços'}</span></h1>{/if}
	...
	{if {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		...
		<div class="address-form-multishipping hidden-xs">
			...
			<div class="button_multishipping_mode" id="multishipping_mode_box">
				...
				<h4 class="title">{l s='Multiplos-envios'}</h4>

				<div class="description">

					<input type="checkbox" id="multishipping_mode_checkbox" onchange="multishippingMode(this); return false;" autocomplete="off"/><label for="multishipping_mode_checkbox">{l s='Eu gostaria de especificar um endereço de entrega para cada produto encomendado.'}</label>

				</div>

				<div class="description_off">

					<a href="{$link->getPageLink('order-opc', true, NULL, 'ajax=1&multi-shipping=1&method=multishipping')|escape:'html'}" id="link_multishipping_form" class="button btn btn-default" title="{l s='Escolha o endereço de entrega(s)'}">

						{l s='Especifique um endereço de entrega para cada produto.'}
...
{/if}
	...
	<div class="addresses clearfix">
		...
		<p class="address_delivery form-group">
			...
			<label for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Escolha um endereço de pagamento:'}{else}{l s='Escolha um endereço de entrega:'}{/if}</label>
		...
		<p class="checkbox-inline addressesAreEquals" {if $cart->isVirtualCart()}style="display:none;"{/if}>
			...
			<label for="addressesAreEquals">{l s='Use o endereço de entrega, como o endereço de cobrança.'}</label>
		...
		{if $addresses|@count > 1}
			...
			<label for="id_address_invoice" class="strong">{l s='Escolha um endereço de pagamento:'}</label>
			...
			{else}
				...
				<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large btn btn-default">{l s='Add um novo endereço'}</a>
		...
		<p class="address_add submit">
			...
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large btn btn-default">{l s='Add um novo endereço'}</a>
...
{if !$opc}
	...
	<p class="cart_navigation submit clearfix">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" title="{l s='Anterior'}" class="button btn btn-default">&laquo; {l s='Anterior'}</a>

		<input type="submit" name="processAddress" value="{l s='Próximo'} &raquo;" class="exclusive btn btn-default" />
