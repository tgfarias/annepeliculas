...
<h1 id="cart_title"><span>{l s='Carrinho de Compras - Resumo'}</span></h1>
...
{if isset($empty)}
	...
	<p class="alert alert-info">{l s='Seu carrinho de compras está vazio.'}<button class="close" data-dismiss="alert" type="button">×</button></p>

{elseif $PS_CATALOG_MODE}

	<p class="alert alert-info">{l s='Esta loja não aceitou sua nova compra.'}<button class="close" data-dismiss="alert" type="button">×</button></p>
...
{else}
	...
	<p style="display:none" id="emptyCartWarning" class="alert alert-info">{l s='Seu carrinho de compras está vazio.'}</p>
...
{if isset($lastProductAdded) AND $lastProductAdded}
	...
	<div class="cart_last_product">
		...
		<div class="cart_last_product_header">
			...
			<div class="left">{l s='Último produto adicionado'}</div>
...
<p class="ordercart-title"><b>{l s='O seu carrinho de compras contém:'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='produto'}{else}{l s='produtos'}{/if}</span></b></p>
		...
		{if $use_taxes}
			...
			{if $priceDisplay}
				...
				<tr class="cart_total_price ">
					...
					<td>{if $display_tax_label}{l s='Total (sem taxas)'}{else}{l s='Total'}{/if}</td>
...
{if $use_taxes && !$priceDisplay}
			...
			{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
				...
				<tr class="cart_total_delivery" style="{if !isset($carrier->id) || is_null($carrier->id)}display:none;{/if}">
					...
					<td  class="price" id="total_shipping">{l s='Envio grátis!'}</td>
				...
				{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
					...
					{if $priceDisplay}
						...
						<tr class="cart_total_delivery" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
							...
							<td >{if $display_tax_label}{l s='Total frete (s/ taxas.)'}{else}{l s='Total frete'}{/if}</td>
					...
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" class="bordercolor product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
						...
						<td class="cart_quantity">
							...
							{else}
								...
								<a rel="nofollow" class="cart_quantity_down" id="cart_quantity_down_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery}&amp;id_customization={$id_customization}&amp;op=down&amp;token={$token_cart}")|escape:'html'}" title="{l s='Retirar'}">
								...
								<a class="cart_quantity_down"  id="cart_quantity_down_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" href="#" title="{l s='Retirar'}">
		...
		{foreach $discounts as $discount}
			...
			<tr class="cart_discount bordercolor {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
                ...
                <div class="insset-bottom">
                	...
                	<span class="title-th">{l s='Preço unitário'}:</span>
                ...
                <span class="title-th">{l s='Qnt.'}:</span>
            ...
            <div id="cart_voucher" class="table_block">
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
                        ...
                        <div class="titled_box">
							...
							<h2 class="title_offers"><span>{l s='Aproveite as nossas ofertas:'}</span></h2>
...
<p id="seperated_packag">
	...
	<label for="allow_seperated_package">{l s='Enviar produtos disponíveis primeiro'}</label>
...
{if !$opc}
	...
	{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
		...
		<p id="seperated_packag" class="checkbox-inline">
			...
			<label for="enable-multishipping">{l s='Gostaria de especificar um endereço de entrega para cada produto.'}</label>
...
<div class="order_delivery row">
	...
	{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
        ...
        <div class="col-xs-12 col-sm-6">
			...
			<li class="address_title">{l s='Endereço para entrega'}&nbsp;<span class="address_alias">({$delivery->alias})</span></li>
        ...
        <div class="col-xs-12 col-sm-6">
			...
			<li class="address_title">{l s='Endereço da fatura'}&nbsp;<span class="address_alias">({$invoice->alias})</span></li>
	...
	{else}
        ...
        <div class="col-xs-12 col-sm-6">
			...
			<ul class="address {if $address@last}last_item{elseif $address@first}first_item{/if} {if $address@index % 2}alternate_item{else}item{/if}">
				...
				<li class="address_title">{if $k eq 'invoice'}{l s='Endereço da fatura'}{elseif $k eq 'delivery' && $delivery->id}{l s='Endereço de Entrega'}{/if}{if isset($address.object.alias)}&nbsp;<span class="address_alias">({$address.object.alias})</span>{/if}</li>
...
<p class="cart_navigation  clearfix inner-top">
	...
	{if !$opc}
		...
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="exclusive btn btn-default standard-checkout" title="{l s='Próximo'}">{l s='Próximo'} &raquo;</a>

		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}

			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="multishipping-button multishipping-checkout exclusive btn btn-default" title="{l s='Next'}">{l s='Próximo'} &raquo;</a>

		{/if}

	{/if}

	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button_large btn btn-default" title="{l s='Continuar comprando'}">&laquo; {l s='Continuar comprando'}</a>
