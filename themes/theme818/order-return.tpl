...
{if isset($orderRet)}
	...
	<div>
		...
		<p class="bold">{l s='Temos registrado o seu pedido de retorno.'}</p>

		<p>{l s='Seu pacote deve ser devolvido para nós dentro'} {$nbdaysreturn} {l s='dias após receber seu pedido.'}</p>

		<p>{l s='O status atual de seu retorno de mercadoria é:'} <span class="bold">{$state_name|escape:'htmlall':'UTF-8'}</span></p>

		<p>{l s='Lista de itens a ser devolvido:'}</p>
		...
		<table id="order-detail-content-table" class="table table-hover table-bordered table-responsive">
			...
			<thead>
				...
				<tr>
					...
					<th class="item">{l s='Produto'}</th>

					<th class="last_item">{l s='Quantidade'}</th>
	...
	<p class="bold">{l s='Lembrete:'}</p>

	<div>

		- {l s='Toda a mercadoria deve ser devolvido em sua embalagem original e em seu estado original.'}
